...
registerAdminController.registerAdmins = async (request, response) => {
  const { name, email, password, isVerified } = request.body;
...
registerAdminController.verifyCode = async (request, response) => {
  ...
  try {
    ...
    return response
      .status(200)
      .json({ message: "Administrador guardado exitosamente" });
  } catch (error) {
    console.log("error " + error);
    return response.status(500).json({ message: "Internal Server Error 500" });
  }
