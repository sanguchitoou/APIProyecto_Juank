...
registerEmployeeController.registerEmployees = async (request, response) => {
...
registerEmployeeController.verifyCode = async (request, response) => {
  ...
  try {
    ...
    //Guardamos TODOS los datos del empleado
    ...
    return response
      .status(200)
      .json({ message: "Empleado guardado exitosamente" });
  } catch (error) {
    console.log("error " + error);
    return response.status(500).json({ message: "Internal Server Error 500" });
  }
